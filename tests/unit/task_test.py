...
from solara.tasks import TaskResult, TaskState, use_task
...
def test_use_task_intrusive_cancel():
    ...
    def Test():
        ...
        result = use_task(work, dependencies=[retry_counter])
    ...
    # also test retry
    while result._state != TaskState.RUNNING:
        time.sleep(0.05)
    assert last_value != 99
    seconds = 0.1
    retry()
    ...
    while result._state in [TaskState.STARTING, TaskState.WAITING, TaskState.RUNNING]:
        time.sleep(0.1)
...
async def test_use_task_async(prefer_threaded):
    ...
    def Test():
        ...
        result = use_task(work, dependencies=[retry_counter], prefer_threaded=prefer_threaded)
        # print("render with", result)
        return w.Label(value="test")
    ...
    retry()
    n = 0
    while result._state == TaskState.CANCELLED:
        await asyncio.sleep(0.1)
        n += 1
        if n == 100:
